# TEST-04: Validaciones de Negocio

## Información del Test

**Clave:** TEST-04  
**Nombre:** Validaciones de Negocio  
**Descripción:** Validar reglas de negocio en operaciones de medicamentos y pacientes  
**Objetivo:** Asegurar que se cumplen todas las reglas de negocio del departamento de farmacia  

## Criterios de Aprobación

- Rechaza medicamentos expirados
- Valida cantidad mínima de stock
- Verifica datos obligatorios de pacientes
- Responde con códigos de error apropiados

## Visualización

| Parámetros | Estatus |
|---|---|
| ![Parámetros](./img/parametros.png) | ![Estatus](./img/estatus.png) |

## Estatus

Pendiente

## Evidencias

### Archivos de Evidencia

- `validacion_medicamentos.log` - Log de validación de medicamentos
- `validacion_pacientes.log` - Log de validación de pacientes
- `casos_prueba.json` - Casos de prueba ejecutados
- `errores_validacion.csv` - Errores encontrados
- `resultado_validaciones.txt` - Resultado de validaciones

### Resultado

Pendiente de ejecución

---

**Fecha de Creación:** Abril 2026
